...
function uploadImage(image) {
    ...
    data.append("query", "summernote_upload_image");
    $.ajax({
        url: api.internal,
        cache: false,
        contentType: false,
...
function body_modal(ref, params){
    ...
    verificaLogin().then(function(logged) {
        ...
        const stringModal = '.modal-dialog'
        $(stringModal).removeClass('modal-xl')
        ...
        switch(ref){
            ...
            case 'campaign':
                ...
                $(stringModal).addClass('modal-xl')

                method = "POST"
                url_api = api.internal

                let statesValues = ``
                Object.keys(statesCitys).forEach((e, i) => {
                    statesValues += `
                ...
                <form id="new-campaign" name="new-campaign">
                    ...
                    <div class="modal-body text-start">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6 form-group my-2">
                                <select disabled class="select2 form-control" name="input-campaign-city" id="input-campaign-city" required>
                                </select>
                            </div>
                        </div>
                ...
                $('#summernote').summernote({
                    placeholder: 'Descreva a campanha o mais objetivo possível',
                    tabsize: 2,
                    height: 100,
                    backColor: "white",
                    toolbar: [
                ...
                $(`input[type="select"][name="input-campaign-location"]`).change(function () {
                ...
                $("#input-campaign-state").on('change', function (){

                    let value = this.value
                    let citys = statesCitys[`${value}`]
                    let citysValues = ``

                    citys.forEach((e, i) => {
                        citysValues += `<option value="${e}">${e}</option>`
                    })

                    $('#input-campaign-city').parent().html(`
                        <select class="select2 form-control" name="input-campaign-city" id="input-campaign-city" required>
                            <option></option>
                            ${citysValues}
                        </select>
                    `)

                    $("#input-campaign-city").select2({
                        placeholder: `Selecione uma Cidade de ${value}`,
                        language: "pt",
                        dropdownParent: $('#modal')
                    })
